...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Pet Store Data Generator for Oracle Database with CSV Export',
...
Examples:
  ...
  python %(prog)s --host localhost --port 1521 --service ORCL --user master
    ...
    csv_group = parser.add_argument_group('CSV Output Configuration')
    csv_group.add_argument('--csv-output-dir',
                           default='./csv_output',
                           help='Directory to save CSV files (default: ./csv_output)')
    csv_group.add_argument('--no-csv', action='store_true',
                           help='Disable CSV file generation')
...
# CSV file locks for thread-safe writing
csv_locks = {
    'suppliers': threading.Lock(),
    'employees': threading.Lock(),
    'customers': threading.Lock(),
    'products': threading.Lock(),
    'pets': threading.Lock(),
    'orders': threading.Lock(),
    'order_items': threading.Lock(),
    'pet_care_logs': threading.Lock(),
    'oracle_datatypes_demo': threading.Lock()
}
...
# --- 4. CSV HELPER FUNCTIONS ---
def setup_csv_files():
    """Create CSV output directory and initialize CSV files with headers."""
    if CONFIG['no_csv']:
        return
    ...
    csv_configs = {
        ...
        'employees': ['employee_id', 'first_name', 'last_name', 'email', 'phone_number', 'hire_date', 'job_title',
                      'salary', 'manager_id'],
        'customers': ['customer_id', 'first_name', 'last_name', 'email', 'phone_number', 'address_line1', 'city',
                      'state', 'zip_code', 'registration_date'],
        'products': ['product_id', 'product_name', 'product_description', 'product_type', 'price', 'cost',
                     'quantity_in_stock', 'supplier_id'],
        'pets': ['pet_id', 'product_id', 'pet_name', 'species', 'breed', 'date_of_birth', 'gender', 'color',
                 'health_status', 'microchip_id', 'adoption_status', 'entry_date'],
        'orders': ['order_id', 'customer_id', 'order_date', 'order_status', 'shipping_address', 'city', 'state',
                   'zip_code', 'payment_method', 'total_amount'],
        'order_items': ['order_item_id', 'order_id', 'product_id', 'quantity', 'unit_price', 'item_total'],
        'pet_care_logs': ['log_id', 'pet_id', 'employee_id', 'log_datetime', 'activity_type', 'notes'],
        'oracle_datatypes_demo': ['id', 'varchar2_column', 'varchar2_large_column', 'nvarchar2_column',
                                  'nvarchar2_large_column',
                                  ...
                                  'timestamp_column', 'timestamp_precision_column', 'timestamp_tz_column',
                                  'timestamp_tz_precision',
                                  'interval_ym_column', 'interval_ym_precision', 'interval_ds_column',
                                  'interval_ds_precision',
                                  'char_column', 'char_large_column', 'nchar_column', 'nchar_large_column']
    }

    # Initialize CSV files with headers
    for table_name, headers in csv_configs.items():
        file_path = os.path.join(CONFIG['csv_output_dir'], f"{table_name}.csv")
    ...
    print(f"CSV files initialized in: {CONFIG['csv_output_dir']}")


def write_to_csv(table_name, data_row):
    """Thread-safe CSV writing function."""
    if CONFIG['no_csv'] or table_name not in csv_files:
        return

    with csv_locks.get(table_name, threading.Lock()):
        try:
            with open(csv_files[table_name], 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(data_row)
        except Exception as e:
            print(f"Error writing to CSV file {table_name}.csv: {e}")


# --- 5. HELPER FUNCTIONS FOR SINGLE INSERTS ---
...
def insert_single_supplier(cur, conn, supplier_index=None):
    ...
    supplier_id = new_id.getvalue()[0]

    # Write to CSV
    csv_data = [supplier_id, name, fake.name(), phone_number, unique_email, address]
    write_to_csv('suppliers', csv_data)
...
def insert_single_employee(cur, conn, manager_id=None, job_title=None, employee_index=None):
    ...
    cur.execute(
        """INSERT INTO employees (first_name, last_name, email, phone_number, hire_date, job_title, salary, manager_id)
           ...
           INTO :9""",
        (first_name, last_name, unique_email, phone_number, hire_date, job, salary, manager_id, new_id)
    )

    employee_id = new_id.getvalue()[0]

    # Write to CSV
    csv_data = [employee_id, first_name, last_name, unique_email, phone_number, hire_date, job, salary, manager_id]
    write_to_csv('employees', csv_data)
...
def insert_single_customer(cur, conn, i):
    ...
    cur.execute(
        """INSERT INTO customers (first_name, last_name, email, phone_number, address_line1, city, state, zip_code,
           ...
           INTO :10""",
        (first_name, last_name, email, phone_number, address_line1, city, state, zip_code, registration_date, new_id)
    )

    customer_id = new_id.getvalue()[0]

    # Write to CSV
    csv_data = [customer_id, first_name, last_name, email, phone_number, address_line1, city, state, zip_code,
                registration_date]
    write_to_csv('customers', csv_data)
...
def insert_single_product(cur, conn, product_type=None, supplier_id=None):
    ...
    product_name = f"Live Pet: {fake.word().title()}" if prod_type == 'Pet' else fake.word().title()
    ...
    cur.execute(
        """INSERT INTO products (product_name, product_description, product_type, price, cost, quantity_in_stock,
           ...
           INTO :8""",
        (product_name, product_description, prod_type, price, cost, quantity_in_stock, supp_id, new_id)
    )

    product_id = new_id.getvalue()[0]

    # Write to CSV
    csv_data = [product_id, product_name, product_description, prod_type, price, cost, quantity_in_stock, supp_id]
    write_to_csv('products', csv_data)
...
def insert_single_pet(cur, conn, product_id):
    ...
    health_status = fake.sentence(nb_words=5)
    microchip_id = fake.ssn()
    ...
    cur.execute(
        """INSERT INTO pets (product_id, pet_name, species, breed, date_of_birth, gender, color, health_status,
           ...
           INTO :12""",
        (product_id, pet_name, species, breed, date_of_birth, gender, color, health_status, microchip_id,
         adoption_status, entry_date, new_id)
    )

    pet_id = new_id.getvalue()[0]

    # Write to CSV
    csv_data = [pet_id, product_id, pet_name, species, breed, date_of_birth, gender, color, health_status, microchip_id,
                adoption_status, entry_date]
    write_to_csv('pets', csv_data)
...
def insert_single_order(cur, conn):
    ...
    for prod_id in order_products:
        ...
        csv_data = [order_item_id_counter, order_id, prod_id, quantity, price, item_total]
        write_to_csv('order_items', csv_data)
        order_item_id_counter += 1
    ...
    # Write order to CSV
    csv_data = [order_id, customer_id, order_date, order_status, address_info[0], address_info[1], address_info[2],
                address_info[3], payment_method, order_total]
    write_to_csv('orders', csv_data)
...
def insert_single_care_log(cur, conn):
    ...
    cur.execute(
        """INSERT INTO pet_care_logs (pet_id, employee_id, log_datetime, activity_type, notes)
           ...
           INTO :6""",
        (pet_id, employee_id, log_datetime, activity_type, notes, new_id)
    )

    log_id = new_id.getvalue()[0]

    # Write to CSV
    csv_data = [log_id, pet_id, employee_id, log_datetime, activity_type, notes]
    write_to_csv('pet_care_logs', csv_data)
...
def insert_single_datatypes_demo_record(cur, conn, i):
    ...
    try:
        ...
        csv_data = [
            demo_id, varchar2_col, varchar2_large_col, nvarchar2_col, nvarchar2_large_col,
            number_col, number_precision_col, number_integer_col,
            float_col, float_precision_col, long_col,
            date_col, binary_float_col, binary_double_col,
            timestamp_col, timestamp_precision_col, timestamp_tz_col, timestamp_tz_precision,
            f"{years_1}-{months_1:02d}", f"{years_2:04d}-{months_2:02d}",
            f"{days_1} {hours_1:02d}:{minutes_1:02d}:{seconds_1:02d}",
            f"{days_2} {hours_2:02d}:{minutes_2:02d}:{seconds_2:02d}.{microseconds_2:06d}",
            char_col, char_large_col, nchar_col, nchar_large_col
        ]
        write_to_csv('oracle_datatypes_demo', csv_data)
...
def initial_setup():
    ...
    try:
        ...
        if not pet_ids or not employee_ids:
            ...
            if care_employees:
                for _ in range(CONFIG['initial_care_logs']):
                    insert_single_care_log(cur, conn)
            else:
        ...
        if not CONFIG['no_csv']:
            print(f"CSV files have been created in: {CONFIG['csv_output_dir']}")
...
if __name__ == "__main__":
    ...
    print("Pet Store Data Generator for Oracle Database with CSV Export")
    print("=" * 60)
    ...
    try:
        ...
        if not CONFIG['no_csv']:
            print(f"Final CSV files are available in: {CONFIG['csv_output_dir']}")
