...
def insert_single_datatypes_demo_record(cur, conn, i):
    ...
    try:
        ...
        float_col = fake.pyfloat(left_digits=5, right_digits=10)  # Example: Adjust as needed
        binary_float_col = fake.pyfloat()  # A simple call generates a valid float
        binary_double_col = fake.pyfloat()  # A simple call generates a valid double
        timestamp_col = datetime.now()
        timestamp_precision_col = datetime.now()
        timestamp_tz_col = datetime.now().astimezone()

        # Generate INTERVAL YEAR TO MONTH data (e.g., '+01-02')
        years = random.randint(0, 99)
        months = random.randint(0, 11)
        interval_ym_col = f"+{years:02}-{months:02}"
        interval_ym_precision = f"+{random.randint(0, 9999):04}-{random.randint(0, 11):02}"

        # Generate INTERVAL DAY TO SECOND data (e.g., '+1 02:03:04.123456')
        days = random.randint(0, 99)
        hours = random.randint(0, 23)
        minutes = random.randint(0, 59)
        seconds = random.uniform(0, 59.999999)
        interval_ds_col = f"+{days:02} {hours:02}:{minutes:02}:{seconds:02.6f}"
        interval_ds_precision = f"+{random.randint(0, 99):02} {random.randint(0, 23):02}:{random.randint(0, 59):02}:{random.uniform(0, 59.999999):06.6f}"

        # Fixed-length strings with padding
        char_col = fake.word().ljust(10)[:10]
        nchar_col = fake.word().ljust(10)[:10]

        # The `long_column` and `rowid_column` cannot be directly inserted.
        # `LONG` is deprecated, and `ROWID` is a pseudo-column generated by Oracle.
        # We will omit them from the insert statement as they are not standard insertable columns.

        # SQL statement
        sql = """INSERT INTO oracle_datatypes_demo (varchar2_column, varchar2_large_column, nvarchar2_column,
                                                    nvarchar2_large_column,
                                                    number_column, number_precision_column, number_integer_column,
                                                    float_column, date_column, binary_float_column,
                                                    binary_double_column,
        ...
        cur.execute(sql, (
            ...
            float_col, date_col, binary_float_col, binary_double_col,
            timestamp_col, timestamp_precision_col, timestamp_tz_col,
